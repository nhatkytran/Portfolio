...
import { layoutCopy, NavigatingProps } from '@/features/layout/data';
  ...
  return (
    <div className="flex items-center gap-2.5">
      <Link href="/" onClick={onTriggerNavigating} className={cn(isNavigating && 'cursor-not-allowed')}>
        <h1 className="font-riot-bold-italic text-lg tracking-widest uppercase">{layoutCopy.englishName}</h1>
      </Link>
      <BringerLabel />
    </div>
  );
